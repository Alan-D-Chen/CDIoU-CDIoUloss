...
def prepare_for_coco_detection(predictions, dataset):
    ...
    #################This module is only used for test dev.#######################
    stamp = time.strftime("%Y%m%d%H%M%S", time.localtime())
    json_file_path = os.path.join("/home/alanc/Documents/ATSS/prediction", stamp + "coco_results2.json")
    json_file = open(json_file_path, mode='a')
    json.dump(coco_results, json_file, indent=4)
    print("This test has been completed, and the prediction results(.json) will be saved in:" + json_file_path)
    ###############################################################################
