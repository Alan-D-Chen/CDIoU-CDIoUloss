...
def inference(
    ...
    # json.dump(save_json_content, json_file, ensure_ascii=False, indent=4)
    ########################################################
    """
    if not is_main_process():
        return
    ...
    ### text-dev ###
    ###  Here is to save the "predictions" result for test-dev! ###
    """
    Here is to compare and evaluate the predicted results with the existing standards
    "evaluate" function returns results, coco_results.
    coco_results:
    coco_results.extend(
            [
                {
    ...
    """
    """
    results, coco_results = evaluate(dataset=dataset, predictions=predictions, output_folder=output_folder, **extra_args)
    """
