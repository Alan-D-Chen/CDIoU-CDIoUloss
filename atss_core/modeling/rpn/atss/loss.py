...
from atss_core.structures.boxlist_ops import boxlist_iou, box_dious
...
class ATSSLossComputation(object):
    ...
    def GIoULoss(self, pred, target, anchor, weight=None):
        ...
        losses = 1 - gious
        """
        New loss function is done here !
        """
        dious = box_dious(anchor, target)
        losses2 = 0.001*dious
        #losses2 = 0.01*dious
        #losses2 = 0.002 * dious
        # print("New loss function is done here !")
        # print(" shape of dious:", dious.shape)
        # print(" type of dious:", type(dious))
        # print(" dious:\n", dious)
        #
        ...
        if weight is not None and weight.sum() > 0:
            ...
            return (losses * weight).sum(), losses2.sum()
        else:
            assert losses.numel() != 0
            # losses = losses + losses2
            return losses.sum(), losses2.sum()
    ...
    def prepare_targets(self, targets, anchors):
        ...
        for im_i in range(len(targets)):
            ...
            if self.cfg.MODEL.ATSS.POSITIVE_TYPE == 'SSC':
                ...
                ###  the ious and dious have changed!
                """
                how to select positves: ATSS (Ours) , SSC (FCOS), IoU (RetinaNet)
                """
                ...
                # ious, dious= boxlist_iou(anchors_per_im, targets_per_im)
    ...
    def __call__(self, box_cls, box_regression, centerness, targets, anchors):
        ...
        if pos_inds.numel() > 0:
            reg_loss, CDIoU_loss = self.GIoULoss(box_regression_flatten, reg_targets_flatten, anchors_flatten,
                                     weight=centerness_targets)
            reg_loss = reg_loss/sum_centerness_targets_avg_per_gpu
            CDIoU_loss = CDIoU_loss/sum_centerness_targets_avg_per_gpu
        ...
        return cls_loss, reg_loss * self.cfg.MODEL.ATSS.REG_LOSS_WEIGHT, CDIoU_loss, centerness_loss
...
def make_atss_loss_evaluator(cfg, box_coder):
    ...
    # print("loss_evaluator:",loss_evaluator)
    # print("information of loss_loss_evaluator:","\n",type(loss_evaluator),"\n",)
